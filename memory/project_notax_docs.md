...
**docs/ organization (Diátaxis):** hand-written content lives in `docs/guias/` (autogenerated sidebar via `{type:'autogenerated', dirName:'guias'}` in `sidebars.ts`, wrapped in a "Guias" category). `docs/notax-pay/` is generated API reference — never edit by hand. New guide = add `.mdx` with frontmatter (`id/title/sidebar_label/sidebar_position/description`); it auto-appears. Convention documented in `CONTRIBUTING.md`.

**Tooling (added 2026-07):** Prettier + ESLint (flat config, eslint pinned to v9 — v10 breaks eslint-plugin-react) + EditorConfig + `.nvmrc` (node 20) + Husky/lint-staged pre-commit. Scripts: `lint`, `format`, `gen-api`, `clean-api`. `onBrokenLinks: 'throw'`. CI at `.github/workflows/ci.yml` runs typecheck/lint/format:check/build on PRs.
